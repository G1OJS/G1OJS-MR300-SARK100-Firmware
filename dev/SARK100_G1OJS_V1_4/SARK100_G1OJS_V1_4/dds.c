...
void DDS_Set(DWORD dwFreqHz)
{
	...
	{									// Needs extended multiply
		muldwu1(dwOp, dwFreqHz, REF_FREQUENCY_NUM);
		dwFreqHz = divluh1(dwOp[0], dwOp[1], REF_FREQUENCY_DEN);
	}

	for (bBitCnt=0; bBitCnt<(sizeof(dwFreqHz)*8); bBitCnt++, dwFreqHz>>=1)
	{
		if ((dwFreqHz&1)!=0)
			DDS_DATA_Data_ADDR |= DDS_DATA_MASK;
		else
